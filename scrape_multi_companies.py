...
# قائمة الشركات المطلوبة (الباقي بعد 4325 و 4322)
COMPANY_SYMBOLS = [
    "4327", "4020", "4090", "4100", "4150", 
    "4220", "4250", "4300", "4310", "4320", 
    "4321", "4326", "4323", "4324", "4230"
]
...
class MultiCompanyScraper:
    ...
    async def scrape_company(self, page: Page, symbol: str) -> Dict[str, Any]:
    ...
    async def scrape_all(self) -> None:
        """Scrapes all companies and saves each to a separate file."""
        ...
        async with async_playwright() as p:
            ...
            try:
                for symbol in COMPANY_SYMBOLS:
                    try:
                        # Scrape company data
                        company_data = await self.scrape_company(page, symbol)
                        
                        # Save to individual file
                        output_file = f"companies_data/{symbol}.json"
                        ...
                        if "error" not in company_data:
                            ...
                            failed += 1
                            print(f"⚠️ Saved with errors to: {output_file}")
            ...
            print(f"📁 Data saved in: companies_data/")
            print(f"{'='*60}")
...
async def main():
    scraper = MultiCompanyScraper(headless=False)
